...
function getCrimeLevels($POLICE, $lat, $lng, $from = null, $to = null) {
    ...
    for ($i = 0; $i <= $diff; $i++) {
        ...
        if (count($crimes) > 0) {
            ...
            $categories = $POLICE->crime_categories($date);
            ...
            foreach ($crimes as $crime) {
                ...
                $category = $data[$date]['categories'][$crime['category']];

                if (isset($category)) {
                    $category->count += 1;
                } else {                
                    $obj = new stdClass();
                    $obj->nicename = $categories[$crime['category']]->nicename;
                    $obj->count = 1;
                    $data[$date]['categories'][$crime['category']] = $obj;
                }
            }
        }
    }
